...
int bmp_log_rm_tlv_path_marking(struct bgp_peer *null1, struct bmp_data *null2, void *vtlv, void *null3, char *null4, int output, void *vobj)
{
  ...
  if (output == PRINT_OUTPUT_JSON) {
    ...
    if (tlv->len == 6 /* status (4) + reason code (2) */) {
	...
	bgp_tag_find((struct id_table *)bmp_logdump_tag.tag_table, &bmp_logdump_tag, &bmp_logdump_tag.tag, NULL);
      ...
      for (afi = AFI_IP; afi < AFI_MAX; afi++) {
        for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi++) {
          ...
          while (node) {
            u_int32_t modulo = bms->route_info_modulo(peer, NULL, NULL, NULL, bms->table_per_peer_buckets);
...
{
  struct p_amqp_host *bmp_daemon_msglog_amqp_host = bdmah;
  ...
  if (!config.bmp_daemon_msglog_amqp_port) config.bmp_daemon_msglog_amqp_port = default_amqp_port;
  ...
  p_amqp_set_port(bmp_daemon_msglog_amqp_host, config.bmp_daemon_msglog_amqp_port);
...
void bmp_dump_init_amqp_host(void *bdah)
{
  ...
  if (!config.bmp_dump_amqp_port) config.bmp_dump_amqp_port = default_amqp_port;
  ...
  p_amqp_set_port(bmp_dump_amqp_host, config.bmp_dump_amqp_port);
...
int bmp_daemon_msglog_init_kafka_host(void *bdmkh)
{
  struct p_kafka_host *bmp_daemon_msglog_kafka_host = bdmkh;
  int ret;
...
avro_schema_t p_avro_schema_build_bmp_rm(int log_type, char *schema_name)
{
  ...
  avro_schema_record_field_append(schema, "is_filtered", avro_schema_int());
...
avro_schema_t p_avro_schema_build_bmp_peer_up(char *schema_name)
{
  ...
  dummy_tlv.type = BMP_PEER_UP_INFO_VRF_TABLE_NAME;
  type = bmp_tlv_type_print(&dummy_tlv, "bmp_peer_up_info", bmp_peer_up_info_types, BMP_PEER_UP_INFO_MAX);
  avro_schema_record_field_append(schema, type, optstr_s);
  free(type);
