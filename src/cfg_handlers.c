...
int cfg_key_aggregate(char *filename, char *name, char *value_ptr)
{
  ...
  while ((count_token = extract_token(&value_ptr, ','))) {
    if (!strcmp(count_token, "src_host")) cfg_set_aggregate(filename, value, COUNT_INT_SRC_HOST, count_token);
    ...
    else if (!strcmp(count_token, "in_cvlan")) cfg_set_aggregate(filename, value, COUNT_INT_IN_CVLAN, count_token);
    else if (!strcmp(count_token, "out_cvlan")) cfg_set_aggregate(filename, value, COUNT_INT_OUT_CVLAN, count_token);
       ...
       !strcmp(count_token, "in_cvlan") || !strcmp(count_token, "out_cvlan")) {
    ...
    else if (!strcmp(count_token, "nvgre")) cfg_set_aggregate(filename, value, COUNT_INT_NVGRE, count_token);
...
int cfg_key_bmp_daemon_msglog_label_filter(char *filename, char *name, char *value_ptr)
{
  struct plugins_list_entry *list = plugins_list;
  int changes = 0;

  for (; list; list = list->next, changes++) {
    changes = load_labels(filename, &list->cfg.bmp_daemon_msglog_label_filter, value_ptr);
  }
